...
public class ElectrDashboardVoltageView extends View {
    ...
    private static final int DEFAULT_ANIM_PLAY_TIME = 1200;//动画时间
    ...
    public ElectrDashboardVoltageView(Context context) {
        this(context, null);
    }

    public ElectrDashboardVoltageView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ElectrDashboardVoltageView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawPointerLine(Canvas canvas){
        ...
        step=valueRange/10;
        int index=0;
        for (int i=minValue; i<maxValue+1; i++){     //一共需要绘制101个表针
            ...
            if (i % step == 0){     //长表针
                ...
                drawPointerText(canvas, i,index);
                index++;
            }else {    //短表针
                pointerPaint.setStrokeWidth(2);
                canvas.drawLine(radiusDial, 0, radiusDial - strokeWidthDial - dp2px(2), 0, pointerPaint);
            }
            canvas.rotate(2.7f*100/valueRange);
        }
    ...
    //画指针文字
    private void drawPointerText(Canvas canvas, int i,int index){
        canvas.save();
        //int currentCenterX = (int) (radiusDial - strokeWidthDial - dp2px(21) - pointerPaint.measureText(String.valueOf(i)) / 2);
        int currentCenterX = (int) (radiusDial - strokeWidthDial - dp2px(15) - pointerPaint.measureText(String.valueOf(i)) / 2);
        canvas.translate(currentCenterX, 0);
        //canvas.rotate(360 - 135 - 2.7f * index);
        //canvas.rotate(360 - 135 - 2.7f * index);        //坐标系总旋转角度为360度
    ...
    private void drawPointer(Canvas canvas){
        ...
        int x1=radiusDial - strokeWidthDial - dp2px(25);
        Log.d(TAG, "drawPointer: x1= "+x1);
        pointerPath.moveTo(x1, 0);
